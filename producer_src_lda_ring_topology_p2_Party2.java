...
public class Party2 implements MessageListener {
    ...
    public void start() {
        ...
        try {
            ...
            while (state == 0) {
                Thread.sleep(1000);
                System.out.println("state in party2 == 0");
            }
            BigInteger en_mu_Attribute1 = new BigInteger(String.valueOf(0));
            BigInteger en_mu_Attribute2 = new BigInteger(String.valueOf(0));

            System.out.println("state == 1");
            // calculate mean's of all attributes, encrypt and send to the next party
            String[] pkMessage = recieveStrMessage.split(",");
            System.out.println("state: " + state);
            n = new BigInteger(pkMessage[0]);
            g = new BigInteger(pkMessage[1]);
            r = randomZStarN();
            ArrayList<BigInteger> means = new ComputationP2().getAttributesMean();
            BigInteger mu_Attribute1 = means.get(0);
            BigInteger mu_Attribute2 = means.get(1);
            en_mu_Attribute1 = encryption.encrypt(mu_Attribute1, n, g, r);
            en_mu_Attribute2 = encryption.encrypt(mu_Attribute2, n, g, r);
            ...
            while (state == 1) {
                Thread.sleep(1000);
                System.out.println("state in party2 == 1");
            }
            ...
            while (flag == 0) {
                Thread.sleep(3000);
                System.out.println("flag==0 in party 2");
            }
